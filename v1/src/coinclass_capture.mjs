...
function isCoinglassHyperliquidUrl(url) {
  return /coinglass\.(com|org)\/(en|pt|fr)\/hyperliquid/i.test(url);
}
...
async function captureCoinglassHyperliquidWhales(page, outDir) {
  const result = await page
    .evaluate(async () => {
      const cleaned = (s) => (s || "").replace(/\s+/g, " ").trim();
      const waitUntil = async (fn, timeoutMs = 30000, intervalMs = 250) => {
      ...
      const table = await waitUntil(() => {
        ...
        for (const t of tables) {
          ...
          const score = rowCount * 100 + tdCount;
          if (score > bestScore) {
            bestScore = score;
            best = t;
          }
        ...
        if (!best) return null;
        const rowCount = best.querySelectorAll("tbody tr").length;
        const firstRow = best.querySelector("tbody tr");
        const tdCount = firstRow ? firstRow.querySelectorAll("td").length : 0;
        if (rowCount < 5 || tdCount < 10) return null;
        return best;
      }, 30000);
      ...
      const parseRows = () => {
        const rows = Array.from(table.querySelectorAll("tbody tr")).map((tr) =>
          Array.from(tr.querySelectorAll("td")).map((td) => cleaned(td.textContent))
        );
        return rows
          .filter((r) => r.length >= 12 && r[1] && r[3])
          .map((r) => ({
            rank: r[1] ?? "",
            user: r[2] ?? "",
            symbol: r[3] ?? "",
      ...
      for (const item of parsed) {
        ...
        top5Unique.push(item);
        if (top5Unique.length >= 5) break;
      }

      return { ok: true, count: parsed.length, top5Unique };
    })
    .catch((e) => ({ ok: false, reason: "evaluate_failed", error: e instanceof Error ? e.message : String(e) }));

  await writeJson(path.join(outDir, "coinglass_hyperliquid_whales.json"), result);
  return result;
}

async function captureCoinglassLiquidationHeatmapModel3(page, outDir) {
  const keywords = ["liquidat", "heat", "heatmap", "liqMap", "liq_map", "model3", "cluster"];
  const captured = [];
  ...
  const handler = async (response) => {
    try {
      ...
      if (!keywords.some((k) => urlLower.includes(k))) return;
      ...
      captured.push({ url, status, contentType: ct, body });
    } catch {}
  };
  ...
  const result = { ok: captured.length > 0, capturedCount: captured.length };
  await writeJson(path.join(outDir, "coinglass_liquidation_heatmap_model3_responses.json"), {
    ...result,
    captured
  });
  return result;
...
const COINGLASS_FEATURE_MAP = [
  {
    key: "hyperliquid",
    name: "Hyperliquid Whale Tracker",
    url: "https://www.coinglass.com/en/hyperliquid",
    api: null
  },
  {
    key: "liquidation_heatmap_model3",
    name: "Liquidation Heatmap Model3",
...
async function main() {
  ...
  try {
    ...
    const outDirDefault = path.resolve("./data/coinclass");
    const TASK_REGISTRY = {
      ...
      "coinglass:hyperliquid:whales": {
        platform: "coinglass",
        name: "CoinGlass Hyperliquid Whale Tracker",
        defaultUrl: "https://www.coinglass.com/en/hyperliquid",
        outDirPrefix: "coinglass_hyperliquid",
        capture: async (page, outDir) => captureCoinglassHyperliquidWhales(page, outDir)
      },
      "coinglass:liquidation_heatmap:model3": {
        platform: "coinglass",
        name: "Liquidation Heatmap Model3",
        defaultUrl: "https://www.coinglass.com/pro/futures/LiquidationHeatMapModel3",
        outDirPrefix: "coinglass_liquidation_heatmap_model3",
        capture: async (page, outDir) => captureCoinglassLiquidationHeatmapModel3(page, outDir)
      }
    };
    ...
    const hasOutDirArg = argv.includes("--outDir");
    const finalArgv = hasOutDirArg 
      ? argv 
      : [...argv, "--outDir", outDirDefault];
    
    session = await createCaptureSession({
      defaultUrl: taskConfig?.defaultUrl ?? "https://coinclass.com/",
      outDirPrefix: taskConfig?.outDirPrefix ?? "coinclass",
    ...
    } else if (isCoinglassHyperliquidUrl(options.url)) {
      const r = await captureCoinglassHyperliquidWhales(page, outDir);
      await writeJson(path.join(outDir, "task_result.json"), { task: "auto:coinglass_hyperliquid", ...r });
    }

    await writeJson(path.join(outDir, "feature_map_blave.json"), BLAVE_FEATURE_MAP);
    await writeJson(path.join(outDir, "feature_map_coinglass.json"), COINGLASS_FEATURE_MAP);

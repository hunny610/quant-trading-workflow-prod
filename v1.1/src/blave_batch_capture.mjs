...
const TASK_REGISTRY = {
  ...
  "blave:tokenized_stock": {
    platform: "blave",
    name: "美股代幣",
    defaultUrl: "https://blave.org/studio/zh/charts/tokenized_stock",
    outDirPrefix: "blave_tokenized_stock",
    capture: async (page, outDir) => {
      await writeJson(path.join(outDir, "task_result.json"), { ok: true, note: "no_api_available" });
      return { ok: true };
    }
  ...
  "blave:crypto_card": {
    platform: "blave",
    name: "加密支付卡",
    defaultUrl: "https://blave.org/studio/zh/charts/crypto_card",
    outDirPrefix: "blave_crypto_card",
    capture: async (page, outDir) => {
      await writeJson(path.join(outDir, "task_result.json"), { ok: true, note: "no_api_available" });
      return { ok: true };
    }
...
async function main() {
  ...
  const pagePoolSizeRaw = getArg("pagePoolSize", "4", process.argv);
  const pagePoolSizeParsed = Number.parseInt(pagePoolSizeRaw, 10);
  const pagePoolSize = Number.isFinite(pagePoolSizeParsed) && pagePoolSizeParsed > 0 ? pagePoolSizeParsed : 4;
    ...
    async function processWithPagePool(taskList, poolSize) {
      ...
      while (index < taskList.length || executing.length > 0) {
        while (index < taskList.length && executing.length < poolSize) {
          const taskKey = taskList[index];
          const workerIndex = index % poolSize;
          
          console.log(`[${index + 1}/${taskList.length}] Scheduling: ${TASK_REGISTRY[taskKey]?.name || taskKey}`);
